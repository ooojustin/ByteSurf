<?php
    ...
    switch ($_GET['action']) {
        ...
        case 'get':
            ...
            // require parameters for title, and type
            $params = array('t', 'type');
            require_get_params($params);
            ...
            $progress = get_progress($username, $_GET['t'], $_GET['type']);
            
            // return progress (season, episode, time, completed)
            if ($progress)
                die(sprintf('%s,%s,%s,%s', $progress['season'], $progress['episode'], $progress['time'], $progress['completed']));
            else
                die('0,0,0,0');
    ...
    function get_progress($username, $title, $type) {
        global $db;
        $get_progress = $db->prepare('SELECT * FROM progress_tracker WHERE username=:username AND title=:title AND type=:type ORDER BY id DESC LIMIT 1');
    ...
    function save_progress($username, $title, $type, $time, $completed, $season = -1, $episode = -1) {
        global $db;
        if (get_progress($username, $title, $type))
            $query = 'UPDATE progress_tracker SET season=:season, episode=:episode, time=:time, completed=:completed WHERE username=:username AND title=:title AND type=:type';
        else
